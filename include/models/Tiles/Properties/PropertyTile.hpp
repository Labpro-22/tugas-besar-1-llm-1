...
public:
  ...
  /// @brief Calculates the amount of rent needed to be paid when the player lands on this property.
  /// @param player The player who landed on the property.
  /// @return The amount of rent to be paid, 0 if the property is not owned by another player or is mortgaged.
  virtual int calculateRent(const Player &player) const = 0;
};

PropertyTile::PropertyTile(const int id, const std::string &code, const std::string &name, const int price) : Tile(id, code, name), price(price)
{
}
